...
public interface SensorHistoryDataMapper extends BaseMapper<SensorHistoryData> {
    @Select("<script>" +
            ...
            "ORDER BY h.dataTime " +
            "</script>")
    Page<SensorHistoryData> pageFindHistoryData(
    ...
    @Select("<script>" +
            "SELECT s.DisplayName as sensorName, p.DisplayName as paramName, p.Sign as paramSign, h.DataTime, h.Value FROM sensorHistoryData h " +
            "LEFT JOIN sensors s ON h.sensorId = s.ID " +
            "LEFT JOIN sensorParameter p ON h.SensorParamId = p.ID " +
            "WHERE 1=1 " +
            "<if test='startTime!= null'>AND h.dataTime BETWEEN #{startTime} AND #{endTime} </if> " +
            "<if test='areaId!= null'>AND s.ComInterfaceId = #{areaId} </if> " +
            "ORDER BY h.dataTime" +
            "</script>")
